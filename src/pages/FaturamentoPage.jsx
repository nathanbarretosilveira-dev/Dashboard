...
export default function FaturamentoPage() {
  ...
  const filtered = faturamentoData.filter(d => {
    const matchSearch = !search || d.motorista.toLowerCase().includes(search.toLowerCase()) || d.rota.toLowerCase().includes(search.toLowerCase()) || d.tomador.toLowerCase().includes(search.toLowerCase()) || d.cte.includes(search);
            ...
            {empresaPie.map((e, i) => (
              <div key={i} className="flex items-center justify-between">
                ...
                <div className="text-right">
                  ...
                  <p className="text-xs text-muted-foreground">{((e.value / totalFat) * 100).toFixed(1)}%</p>
                </div>
              </div>
            ))}
          </div>
      ...
      <div className="bg-card rounded-xl border border-border overflow-hidden">
        <div className="p-5 border-b border-border">
          <div className="flex flex-col sm:flex-row gap-3 items-start sm:items-center justify-between">
            <h2 className="font-semibold text-foreground text-sm">CTes Emitidos</h2>
          ...
          <table className="w-full text-xs">
            <thead>
              <tr className="bg-muted/50 text-muted-foreground uppercase tracking-wider">
                ...
                <th className="text-left px-4 py-3 font-semibold hidden lg:table-cell">Tomador</th>
                <th className="text-right px-4 py-3 font-semibold">Qtd (L)</th>
                <th className="text-right px-4 py-3 font-semibold">Valor Total</th>
                <th className="text-right px-4 py-3 font-semibold hidden lg:table-cell">Pedágio</th>
            ...
            <tbody className="divide-y divide-border">
              {filtered.map((d, i) => (
                <tr key={i} className="hover:bg-muted/30 transition-colors">
                  ...
                  <td className="px-4 py-3 text-muted-foreground hidden md:table-cell truncate max-w-[120px]">{d.motorista}</td>
                  <td className="px-4 py-3">
                    <span className="px-2 py-0.5 bg-muted rounded-full text-xs font-medium">{d.rota}</span>
                  </td>
                  <td className="px-4 py-3 text-muted-foreground hidden lg:table-cell truncate max-w-[120px]">{d.tomador}</td>
                  ...
                  <td className="px-4 py-3 text-right text-muted-foreground hidden lg:table-cell">{d.pedagio > 0 ? fmt(d.pedagio) : '—'}</td>
        ...
        <div className="px-4 py-3 border-t border-border bg-muted/30">
          <p className="text-xs text-muted-foreground">{filtered.length} CTes exibidos · Total: <strong className="text-foreground">{fmt(filtered.reduce((s, d) => s + d.valorTotal, 0))}</strong></p>
        </div>
      </div>
    </div>
  );
